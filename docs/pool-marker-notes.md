...
No Appearance, Show object, Programmer state or native update state is changed by marking. Markers are removed when references change, the tool stops or POOL BLINK is turned off. Selection/pool scrolling is rescanned. Recall View can retain valid-but-hidden grid handles, so v0.7.0.17 checks `IsActuallyVisible()` and performs one bounded rediscovery when a cached grid becomes hidden or invalid. Generator command-address aliases fall back to native-object identity. Runtime rendering of frames attached to native pool tiles still requires grandMA3 validation. If a native tile rejects the child frame, the plugin skips it.
