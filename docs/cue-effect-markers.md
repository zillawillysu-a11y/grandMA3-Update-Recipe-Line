...
v0.7.0.14 uses three resolver tiers. FAST publishes Phaser Recipes/Generators stored directly in the current Cue before the first yield. MEDIUM runs on the next host tick and walks only the bounded Cue/Part/Recipe object tree; newest Recipe per exact Group and feature wins, so inherited Phaser Recipe references publish without `GetPresetData`, while a newer static Recipe in the same lane terminates them. SLOW then reads cooked channel data in batches and replaces the progressive MEDIUM result with its authoritative snapshot; if SLOW aborts, the useful progressive result remains. This ordering guarantees a UI paint opportunity between MEDIUM and the first potentially expensive native read. MEDIUM is intentionally provisional because overlapping Groups, releases and manually stored channels require SLOW.

All Recipe readers accept both native object handles and address strings. Address strings from `recipe:Get("SELECTION")`, `recipe:Get("VALUES")`, or `recipe:Get("GENERATOR")` are resolved through `ObjectList`, matching the behavior proven by the external Recipes Highlight plugin. v0.7.0.12 assumed handles in this new path and produced no purple frames in the user's real Show. MEDIUM now logs `progressive cue=... refs=N` or `progressive_abort` to Command Line History.

Runtime state records its plugin version. Executing an imported newer version stops and replaces the old instance in one invocation; executing the same version remains the intentional ON/OFF toggle. Previously, the first execution after an import could only stop the old instance and leave all purple frames absent until the user executed the plugin a second time.
