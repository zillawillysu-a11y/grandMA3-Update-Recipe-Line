...
- Diagnostic 0.1.5.0 confirmed that Group `Selection` is a structured table containing each member's zero-based `sf_index` plus grid coordinates. This enables a strictly read-only reverse match against `SelectionFirst/Next` output. Its textual table printer truncates after 80 fields, so fixture counts and set equality must be computed from the live Lua table rather than reconstructed from the bounded Dump text. The user confirmed Group 9 contains the full 20-fixture selection; diagnostic 0.1.6.0 performs the required live-table comparison.
- Diagnostic 0.1.6.0 scanned 63 Groups and found three exact 20-fixture set matches: Group 2 `S TOP ALL`, Group 9 `SPOT TOP GRID`, and Group 261 `S TOP SNAKE`. Fixture membership alone is therefore insufficient. Official `SelectionFirst/Next` grid returns and each Group's stored grid allow a second-stage normalized X/Y/Z fingerprint comparison; diagnostic 0.1.7.0 adds this without changing selection state.
- A 0.1.11.0 single-fixture test selected patch index 79 from Group 9 and correctly returned no exact-set match. Subset support must be Recipe-first: a Recipe is a candidate only if its Selection Group contains every selected fixture and its Values feature matches the Programmer feature. Diagnostic 0.1.12.0 implements this and reports selected/Group fixture coverage; uniqueness is decided across matching Recipes, not arbitrary Groups.
